...
public class KmeansRun {
     ...
     * kNum             簇的个数
     * iterNum          迭代次数
     * iterMaxTimes     单次迭代最大运行次数
     * disDiff          单次迭代终止条件，两次运行中类中心的距离差
     * original_data    原始数据集
     * pointList        原始数据集所构建的点集
     * len              点的维度
     */
    private int kNum;
    ...
    private List<Desc> originalData = null;
    private static List<Point> pointList = null;
    private DistanceCompute disC = new DistanceCompute();
    private int len = 0;

    public KmeansRun(int kNum, List<Desc> originalData) {
        this.kNum = kNum;
        this.originalData = originalData;
        this.len = originalData.get(0).getLoc().length;
        //检查规范
        check();
        //初始化点集。
        init();
    ...
    private void init() {
        ...
        for (int i = 0, j = originalData.size(); i < j; i++) {
            Desc desc = originalData.get(i);
            pointList.add(new Point(i, desc.getLoc(), desc.getLable()));
        }
    }
    ...
    public boolean calculateCenter(Set<Cluster> clusterSet) {
        ...
        for (Cluster cluster : clusterSet) {
            ...
            // 计算两个新、旧中心的距离，如果任意一个类中心移动的距离大于dis_diff则继续迭代。小于则中心点没有移动 终止迭代
